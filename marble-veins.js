...
(function() {
  ...
  function createNoise(seed) {
    ...
    return function(x, y) {
      ...
      return lerp(lerp(gx[aa & 255] * xf + gy[aa & 255] * yf, gx[ba & 255] * (xf - 1) + gy[ba & 255] * yf, u), lerp(gx[ab & 255] * xf + gy[ab & 255] * (yf - 1), gx[bb & 255] * (xf - 1) + gy[bb & 255] * (yf - 1), u), v);
    };
  }
  ...
  function doubleWarp(noise, noise2, x, y, oct, str) {
    ...
    var w2 = domainWarp(noise2, x + 1.7, y + 3.2, oct, str * .7);
    return w1 * .65 + w2 * .35;
  }
  var themes = {
    calacatta: {
  ...
  function pickC() {
    return cfg.colors[_rand() * cfg.colors.length | 0];
  }
  function genVein(noise, noise2, idx, pw, ph, primary) {
    var aRad = (cfg.angle + idx * (primary ? 7 : 13) + (_rand() - .5) * 12) * Math.PI / 180;
    ...
    var frac = (idx + _rand() * .5) / total;
    var diag = Math.sqrt(pw * pw + ph * ph);
    var perpOff = frac * diag * 1.15 - diag * .075;
    var wFreq = 2e-4 + _rand() * 8e-4;
    var wAmp = primary ? 60 + _rand() * 140 : 25 + _rand() * 80;
    var warpAmt = cfg.warp * (primary ? 1 : .6);
    var pts = [];
    var len = diag * 1.7;
    ...
    for (var s = 0; s <= len; s += step) {
      var t = s / len;
      var along = s - len * .35;
      var n1 = fbm(vn, t * 3.2, idx * 2.5, 5, 2.1, .48);
      var n2 = fbm(vn2, t * 4.5 + idx * 1.3, idx * 3 + .7, 4, 2.3, .45);
      var warp = doubleWarp(noise, noise2, t * 2.2 + idx * .6, idx * 1.1, 4, warpAmt);
      var wave = Math.sin(along * wFreq + idx * 1.1) * wAmp;
      var wave2 = Math.sin(along * wFreq * 2.7 + idx * .8 + 1.5) * wAmp * .3;
      var perp = perpOff + wave + wave2 + n1 * 160 + n2 * 80 + warp * 120;
      var px = along * cosA - perp * sinA + pw * .5;
      var py = along * sinA + perp * cosA + ph * .5;
      var tv = .15 + .85 * Math.pow(.5 + .5 * Math.sin(t * Math.PI * (1.3 + _rand() * 2.5) + idx * .9), 1.6);
      var tnoise = fbm(vn, t * 8, idx * 5.3, 3) * .3;
      tv = Math.max(.08, tv + tnoise);
      var tap = 1;
      if (t < .05) tap = Math.pow(t / .05, .7); else if (t > .95) tap = Math.pow((1 - t) / .05, .7);
      tv *= tap;
      pts.push({
        x: px,
        y: py,
  ...
  function genBranches(mainPts, idx) {
    ...
    for (var b = 0; b < nb; b++) {
      var bi = Math.floor((.08 + _rand() * .78) * mainPts.length);
      var o = mainPts[Math.min(bi, mainPts.length - 1)];
      var ba = (cfg.angle + idx * 7) * Math.PI / 180 + (_rand() - .5) * 2;
      var bl = 50 + _rand() * 350;
      var bn = createNoise(cfg.seed + idx * 31 + b * 17 + 3e3);
      ...
      for (var s = 0; s < bl; s += 1.5) {
        ...
        bx += Math.cos(ba + (noise1 + noise2val) * .005) * 1.5;
        by += Math.sin(ba + (noise1 + noise2val) * .005) * 1.5;
        var tap = Math.pow(1 - bt, 1.5) * Math.min(1, bt * 6) * (.8 + .2 * Math.sin(bt * Math.PI));
        pts.push({
          x: bx,
          y: by,
          t: bt,
      ...
      if (_rand() > .4 && pts.length > 25) {
        var sbi = Math.floor((.2 + _rand() * .5) * pts.length);
        var sbo = pts[Math.min(sbi, pts.length - 1)];
        var sba = ba + (_rand() - .5) * 1.8;
        var sbl = 25 + _rand() * 100;
        var sbn = createNoise(cfg.seed + idx * 31 + b * 41 + 4e3);
        var sub = [];
        var sbx = sbo.x, sby = sbo.y;
        for (var ss = 0; ss < sbl; ss += 1.5) {
          var st = ss / sbl;
          var sn = fbm(sbn, st * 5, b * 3, 2) * 22;
          sbx += Math.cos(sba + sn * .007) * 1.5;
          sby += Math.sin(sba + sn * .007) * 1.5;
          sub.push({
            x: sbx,
            y: sby,
            t: st,
            th: Math.pow(1 - st, 2) * Math.min(1, st * 8) * .7
          });
        }
        brs.push(sub);
      }
  ...
  function genMicro(noise, pw, ph) {
    ...
    for (var m = 0; m < cnt; m++) {
      ...
      var l = 25 + _rand() * 180;
      var mn = createNoise(cfg.seed + m * 41 + 5e3);
      var pts = [];
      for (var s = 0; s <= l; s += 1.2) {
        var t = s / l;
        var n = fbm(mn, t * 7, m * 1.5, 3) * 20;
        pts.push({
          x: sx + Math.cos(a + n * .01) * s,
          y: sy + Math.sin(a + n * .01) * s,
          t: t,
          th: Math.sin(t * Math.PI) * (.3 + _rand() * .7) * Math.pow(Math.sin(t * Math.PI), .3)
        });
      }
      paths.push(pts);
    }
  ...
  function drawSmoothVein(ctx, pts, baseW, color, alpha, glowR, ox, oy) {
    ...
    for (var i = 0; i < pts.length; i++) {
      ...
      sa.push(alpha * (.5 + .5 * pts[i].th));
    }
    if (glowR > 8) {
      ctx.save();
      ctx.globalAlpha = alpha * .08 * cfg.glow;
      ctx.strokeStyle = rgba(color, 1);
      ctx.lineWidth = baseW * .7 + glowR * 4;
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      ...
      for (var g1 = 1; g1 < sx.length; g1 += 8) {
        ...
        ctx.quadraticCurveTo(sx[g1], sy[g1], (sx[g1] + sx[g1n]) * .5, (sy[g1] + sy[g1n]) * .5);
      }
      ctx.stroke();
      ctx.restore();
    }
    if (glowR > 0) {
      ctx.save();
      ctx.globalAlpha = alpha * .2 * cfg.glow;
      ctx.strokeStyle = rgba(color, 1);
      ctx.lineWidth = baseW * .5 + glowR * 1.5;
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      ctx.filter = "blur(" + Math.round(glowR * .6) + "px)";
      ctx.beginPath();
      ctx.moveTo(sx[0], sy[0]);
      for (var g2 = 1; g2 < sx.length; g2 += 4) {
        var g2n = Math.min(g2 + 2, sx.length - 1);
        ctx.quadraticCurveTo(sx[g2], sy[g2], (sx[g2] + sx[g2n]) * .5, (sy[g2] + sy[g2n]) * .5);
      }
      ctx.stroke();
      ctx.restore();
    }
    ...
    for (var seg = 0; seg < sx.length - 1; seg += Math.max(1, Math.floor(segLen / 2))) {
      ...
      for (var k = seg + 1; k <= segEnd; k++) {
        if (k < segEnd) {
          var cpx = sx[k];
          var cpy = sy[k];
          var epx = (sx[k] + sx[k + 1]) * .5;
          var epy = (sy[k] + sy[k + 1]) * .5;
    ...
    ctx.restore();
    if (baseW > 2.5) {
      var hlColor = [ Math.min(255, color[0] + 50), Math.min(255, color[1] + 50), Math.min(255, color[2] + 50) ];
      ctx.save();
      ...
      for (var hl = 0; hl < sx.length - 2; hl += Math.max(1, Math.floor(segLen / 2))) {
        ...
        hlA /= hlCnt;
        var coreW = hlW * .25;
        if (coreW < .25) continue;
        ctx.globalAlpha = hlA * .3;
        ctx.lineWidth = coreW;
        ctx.beginPath();
        ctx.moveTo(sx[hl], sy[hl]);
        for (var hk = hl + 1; hk <= hlEnd; hk++) {
          if (hk < hlEnd) {
            ctx.quadraticCurveTo(sx[hk], sy[hk], (sx[hk] + sx[hk + 1]) * .5, (sy[hk] + sy[hk + 1]) * .5);
          } else {
  ...
  function drawAmbient(ctx, w, h, elTop, pageH) {
    ...
    for (var p = 0; p < cfg.patches; p++) {
      ...
      var rad = Math.min(w, h) * (.2 + _rand() * .55);
      var c = _rand() > .4 ? cfg.ambient : pickC();
      var nv = (fbm(an, (px + elTop) * 8e-4, (py + elTop) * 8e-4, 4) + 1) * .5;
      var op = .03 + nv * .055;
      var gr = ctx.createRadialGradient(px, py, 0, px, py, rad);
      gr.addColorStop(0, rgba(c, op * 1.4));
      gr.addColorStop(.2, rgba(c, op));
      gr.addColorStop(.5, rgba(c, op * .55));
      gr.addColorStop(1, rgba(c, 0));
      ctx.fillStyle = gr;
      ctx.fillRect(0, 0, w, h);
    }
    for (var t = 0; t < 5; t++) {
      var tx = _rand() * w * .6 + w * .2;
      var ty = _rand() * h * .6 + h * .2;
      var tr = Math.max(w, h) * (.4 + _rand() * .6);
      var tc = pickC();
      var tg = ctx.createRadialGradient(tx, ty, 0, tx, ty, tr);
      tg.addColorStop(0, rgba(tc, .022));
      tg.addColorStop(.5, rgba(tc, .01));
      tg.addColorStop(1, rgba(tc, 0));
      ctx.fillStyle = tg;
      ctx.fillRect(0, 0, w, h);
    }
    var lgAngle = cfg.angle * Math.PI / 180;
    var lx1 = w * .5 - Math.cos(lgAngle) * w * .5;
    var ly1 = h * .5 - Math.sin(lgAngle) * h * .5;
    var lx2 = w * .5 + Math.cos(lgAngle) * w * .5;
    var ly2 = h * .5 + Math.sin(lgAngle) * h * .5;
    var lg = ctx.createLinearGradient(lx1, ly1, lx2, ly2);
    var lc = cfg.ambient;
    lg.addColorStop(0, rgba(lc, .012));
    lg.addColorStop(.3, rgba(lc, .022));
    lg.addColorStop(.7, rgba(lc, .008));
    lg.addColorStop(1, rgba(lc, .014));
    ctx.fillStyle = lg;
    ctx.fillRect(0, 0, w, h);
  }
  function drawFades(ctx, w, h, fTop, fBot) {
    if (fTop > 0) {
      ...
      gt.addColorStop(.4, "rgba(0,0,0,0.5)");
    ...
    if (fBot > 0) {
      ...
      gb.addColorStop(.6, "rgba(0,0,0,0.5)");
    ...
    zones.forEach(function(zone, zi) {
      ...
      micros.forEach(function(mp) {
        ...
        for (var m = 0; m < mp.length; m += 4) {
          if (mp[m].y >= elTop - 60 && mp[m].y <= elTop + h + 60) {
            hit = true;
            break;
          }
        }
        if (!hit) return;
        drawSmoothVein(ctx, mp, .6, pickC(), rr(cfg.microA[0], cfg.microA[1]), 0, 0, elTop);
      });
      sPaths.forEach(function(sp) {
        drawSmoothVein(ctx, sp, rr(cfg.sThick[0], cfg.sThick[1]), pickC(), rr(cfg.sAlpha[0], cfg.sAlpha[1]), rr(4, 12), 0, elTop);
      });
      branches.forEach(function(bp) {
        drawSmoothVein(ctx, bp, rr(cfg.sThick[0], cfg.sThick[1]) * .6, pickC(), rr(cfg.sAlpha[0], cfg.sAlpha[1]) * .7, rr(2, 7), 0, elTop);
      });
      pPaths.forEach(function(pp) {
        drawSmoothVein(ctx, pp, rr(cfg.pThick[0], cfg.pThick[1]), pickC(), rr(cfg.pAlpha[0], cfg.pAlpha[1]), rr(8, 22), 0, elTop);
      });
      var fadeAmt = Math.min(120, h * .3);
      var fTop = fadeAmt;
      var fBot = fadeAmt;
      if (zi === 0) fTop = fadeAmt * .2;
